...
In_Heater_Temp_Setup = 1;
    ...
    while (In_Heater_Temp_Setup == 1)
    {
		...
		// add code to handle 'Play/Pause', 'Minus', 'Plus' and 'EQ'
                if (found == Play_Pause) Do_Save_New_Heater_Temp();    // 'Play/Pause' then use  Do_Save_New_Fan_Temp();                 
                if (found == Minus) Decrease_Heater_Temp();            // 'Minus' then use Decrease_Fan_Temp();
                if (found == Plus) Increase_Heater_Temp();             // 'Plus' then use Increase_Fan_Temp(); 
                if (found == EQ) Exit_Setup_Heater_Temp();             // 'EQ' then use Exit_Setup_Fan_Temp();
...
void Increase_Heater_Temp()
{
    ...
    if (setup_heater_set_temp > 120) setup_heater_set_temp = 120;
...
{ 
    ...
    strcpy(txt, "ECE3301L Sp'22 S3");                    // Text displayed 
